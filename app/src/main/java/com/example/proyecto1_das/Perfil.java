...
public class Perfil extends AppCompatActivity {
    ...
    private void askCameraPermissions(){
        if (ContextCompat.checkSelfPermission(Perfil.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED){
            ActivityCompat.requestPermissions(Perfil.this, new String[] {Manifest.permission.CAMERA}, CAMERA_REQUEST_CODE);
        }else{
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == CAMERA_PERM_CODE){
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                ...
                Toast.makeText(Perfil.this, "Camara", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private void uploadImageToFirebase(String name, Uri contentUri) {
        ...
        image.putFile(contentUri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            ...
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(Perfil.this, "Fallo en la subida", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void verImagen(Uri uri){
        ...
        if (uri != null){
            pulsado = true;
            imagen = findViewById(R.id.imagenCamara);
            Picasso.get().load(uri).into(imagen); //Se utiliza para descargar la imagen
            onTokenRefresh();
        }
        else {
            Toast.makeText(Perfil.this, "Debería sacar una foto", Toast.LENGTH_SHORT).show();
        }
    }
